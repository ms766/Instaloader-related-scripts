...
while [[ $input != '' ]]; do
  ...
  if [[ $input == 'all' ]]; then
    ...
    for each in "${olist[@]}"
      do
        ...
        open "~/Pictures/0gram/$each/0Pictures"
        cd "~/Pictures/0gram/$each/0clips"
        if [[ `ls -1 |grep ."mp4" | wc -l` -gt 0 ]]; then
          cd .. &&   open "~/Pictures/0gram/$each/0clips"
    ...
    elif [[ $input == range ]]; then echo -n ${red}"Enter start-point:"${reset} && read y; for KEY in "${!olist[@]}"; do if [[ $KEY -ge $y ]]; then open "~/Pictures/0gram/${olist[$KEY]}/0Pictures"; fi; cd "~/Pictures/0gram/${olist[$KEY]}/0clips"; if [[ `ls -1 |grep ."mp4" | wc -l` -gt 0 ]]; then cd .. && open "~/Pictures/0gram/${olist[$KEY]}/0clips"; cd ..; fi done && echo ${red}'opening PIC DIRS in range'${reset} && exit
  ...
  [[ $input != '' && -n ${olist[$input]} ]] && open "~/Pictures/0gram/${olist[$input]}" || printf '\n'${red}"bad id error - Try again or hit enter to exit"${reset}'\n\n'
